...
export default class PropertyFieldTermSetPickerHost extends React.Component<IPropertyFieldTermSetPickerHostProps, IPropertyFieldFontPickerHostState> {
  ...
  private getSelectedNodePosition(node: ISPTermSet): number {
    for (var i = 0; i < this.state.activeNodes.length; i++) {
      if (node.Guid === this.state.activeNodes[i].Guid) return i;
    }
    return -1;
  }
  ...
  private renderNode(node: ISPTermObject): JSX.Element {
    ...
    if (isFolder === true) checkBoxAvailable = false;
    var picUrl: string = "";
    ...
    return (
      <div style={style} onClick={this.onClickNode.bind(null, node)} id={node.Guid} role="menuitem">
        ...
        <div style={{ paddingTop: "7px" }}>
          {picUrl !== undefined && picUrl != "" ? <img src={picUrl} width="18" height="18" style={{ paddingRight: "5px" }} alt={node.Name} /> : ""}
      ...
      <div>
        ...
        >
          ...
          {this.state.loaded === true
            ? this.state.termStores.map((rootNode: ISPTermStore, index: number) => {
                return (
                  <Tree
                    paddingLeft={15}
                    tree={rootNode}
...
class SPTermStorePickerService {
      ...
      return this.context.spHttpClient
        ...
        .then((response: SPHttpClientResponse) => {
          return response.json().then((jsonResponse: any) => {
            ...
            return this.context.spHttpClient
              .post(clientServiceUrl, SPHttpClient.configurations.v1, httpPostOptions)
              .then((serviceResponse: SPHttpClientResponse) => {
                    ...
                    if (child != null && child["_ObjectType_"] !== undefined) {
                      ...
                      if (objType === "SP.Taxonomy.TaxonomySession") {
                        ...
                        childTermStores.map((childTerm: any) => {
                          ...
                          if (!(this.props.excludeOfflineTermStores === true && newTermStore.IsOnline === false)) res.push(newTermStore);
                        });
                      }
                    }
                  });
  ...
  public getTermStoresGroups(termStore: ISPTermStore): Promise<ISPTermGroups> {
    if (Environment.type === EnvironmentType.Local) {
      ...
      var clientServiceUrl = this.context.pageContext.web.absoluteUrl + "/_vti_bin/client.svc/ProcessQuery";
      var data =
        '<Request xmlns="http://schemas.microsoft.com/sharepoint/clientquery/2009" SchemaVersion="15.0.0.0" LibraryVersion="16.0.0.0" ApplicationName="Javascript Library"><Actions><ObjectPath Id="16" ObjectPathId="15" /><Query Id="17" ObjectPathId="15"><Query SelectAllProperties="true"><Properties /></Query><ChildItemQuery SelectAllProperties="true"><Properties /></ChildItemQuery></Query></Actions><ObjectPaths><Property Id="15" ParentId="5" Name="Groups" /><Identity Id="5" Name="' +
        termStore.Identity +
        '" /></ObjectPaths></Request>';
      var httpPostOptions = {
        headers: {
                ...
                if (child["_Child_Items_"] !== undefined) {
                  child["_Child_Items_"].map((childGroup: any) => {
                    ...
                    if (this.props.excludeSystemGroup === true) {
                      if (objGroup.IsSystemGroup !== true) res.push(objGroup);
                    } else {
                      res.push(objGroup);
                    }
                  });
  ...
  public getTermSets(termStore: ISPTermStore, group: ISPTermGroup): Promise<ISPTermSets> {
    if (Environment.type === EnvironmentType.Local) {
      ...
      var clientServiceUrl = this.context.pageContext.web.absoluteUrl + "/_vti_bin/client.svc/ProcessQuery";
      var data =
        '<Request xmlns="http://schemas.microsoft.com/sharepoint/clientquery/2009" SchemaVersion="15.0.0.0" LibraryVersion="16.0.0.0" ApplicationName="Javascript Library"><Actions><ObjectPath Id="26" ObjectPathId="25" /><ObjectIdentityQuery Id="27" ObjectPathId="25" /><ObjectPath Id="29" ObjectPathId="28" /><Query Id="30" ObjectPathId="28"><Query SelectAllProperties="true"><Properties /></Query><ChildItemQuery SelectAllProperties="true"><Properties /></ChildItemQuery></Query></Actions><ObjectPaths><Method Id="25" ParentId="15" Name="GetById"><Parameters><Parameter Type="String">' +
        group.Guid +
        '</Parameter></Parameters></Method><Property Id="28" ParentId="25" Name="TermSets" /><Property Id="15" ParentId="5" Name="Groups" /><Identity Id="5" Name="' +
        termStore.Identity +
        '" /></ObjectPaths></Request>';
      var httpPostOptions = {
        headers: {
                ...
                if (child["_Child_Items_"] !== undefined) {
                  child["_Child_Items_"].map((childGroup: any) => {
                    ...
                    if (this.props.displayOnlyTermSetsAvailableForTagging === true) {
                      if (objGroup.IsAvailableForTagging === true) res.push(objGroup);
                    } else {
                      res.push(objGroup);
                    }
                  });
  ...
  private getTermStoresFromMock(): Promise<ISPTermStores> {
    return SPTermStoreMockHttpClient.getTermStores(this.context.pageContext.web.absoluteUrl).then(() => {
      const mockData: ISPTermStores = [
        {
          Name: "Taxonomy_jHIKWt45FAQsxsbHfZ3r1Q==",
          Guid: "/Guid(8ca33abb-2ee5-42d4-acb6-bd138adec078)/",
    ...
    return SPTermStoreMockHttpClient.getTermStoresGroups(this.context.pageContext.web.absoluteUrl).then(() => {
      const mockData: ISPTermGroups = [
        ...
        },
        {
          ...
          Guid: "/Guid(8ca33acc-2ee5-42d4-acb6-bd138adec078)/",
          Identity: "8ca33abb-2ee5-42d4-acb6-bd138adec078",
          IsSiteCollectionGroup: false,
          IsSystemGroup: false,
          CreatedDate: "",
  ...
  private getTermSetsFromMock(termStoreIdentity: string, groupGuid: string): Promise<ISPTermSets> {
    return SPTermStoreMockHttpClient.getTermSetsFromMock(this.context.pageContext.web.absoluteUrl).then(() => {
      const mockData: ISPTermSets = [
        {
          Name: "People",
          Guid: "/Guid(8ca44acc-2ee5-42d4-acb6-bd138adec078)/",
        ...
        {
          Name: "Job Title",
          Guid: "/Guid(8ca44acc-2ff4-42d4-acb6-bd138adec078)/",
