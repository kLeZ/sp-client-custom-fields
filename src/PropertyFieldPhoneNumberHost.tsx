    ...
    return (
      <div>
        <span style={shellStyle}>
          <span style={shellStyleSpan} aria-hidden="true" ref="spanMask" id={props.id + "Mask"}>
            <i style={shellStyleSpanI}>{this.state.value}</i>
            {placeHolderContent}
          </span>
          <input
            style={inputShell}
...
export default class PropertyFieldPhoneNumberHost extends React.Component<IPropertyFieldPhoneNumberHostProps, {}> {
  ...
  public render(): JSX.Element {
    ...
    if (this.props.phoneNumberFormat != null) selectedFormat = this.props.phoneNumberFormat;
    var selectedPattern: IPhoneNumberFormatPattern;
    for (var i = 0; i < this.patterns.length; i++) {
      if (this.patterns[i].type === selectedFormat) selectedPattern = this.patterns[i];
    }
