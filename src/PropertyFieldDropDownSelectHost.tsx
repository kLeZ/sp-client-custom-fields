 ...
 * Renders the controls for PropertyFieldDropDownSelect component
 */
export default class PropertyFieldDropDownSelectHost extends React.Component<
  IPropertyFieldDropDownSelectHostProps,
  IPropertyFieldDropDownSelectHostState
> {
  private async: Async;
  ...
  constructor(props: IPropertyFieldDropDownSelectHostProps) {
    ...
    if (props.initialValue != null && props.initialValue.length > 0 && this.props.options != null) {
      for (var i = 0; i < this.props.options.length; i++) {
        ...
        if (found == true) font.isSelected = true;
      }
    }
  }
  ...
  private saveOptions(): void {
    ...
    this.props.options.map((elm: IDropdownOption) => {
      if (elm.isSelected) res.push(elm.key.toString());
    });
              ...
              {this.props.options.map((elm: IDropdownOption, index?: number) => {
                if (elm.isSelected) {
                  if (foundSelected == false) {
                    foundSelected = true;
                    return <span key={this._key + "-spanselect-" + index}>{elm.text}</span>;
                  } else {
                    return <span key={this._key + "-spanselect-" + index}>, {elm.text}</span>;
                  }
                }
          ...
          <div style={fsDrop}>
            <ul style={fsResults}>
              {this.props.options.map((font: IDropdownOption, index: number) => {
                ...
                if (this.state.hoverFont === font.text) backgroundColor = "#eaeaea";
                var innerStyle = {
                  lineHeight: "80%",
                  padding: "7px 7px 8px",
                  margin: "0",
